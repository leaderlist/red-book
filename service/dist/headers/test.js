...
exports.getXHeader = void 0;
const dom_storage_1 = require("dom-storage");
const X_Data_1 = require("../X-Data");
const constants_1 = require("./constants");
const getSignParams_1 = require("./getSignParams");
var HTMLCanvasElement = X_Data_1.window.HTMLCanvasElement;
HTMLCanvasElement.prototype.getContext = function () {
    return {};
};
X_Data_1.window.document = {
    ...X_Data_1.document,
    cookie: `a1=${(0, X_Data_1.generateLocalId)('Mac OS')}`,
...
async function getXHeader(url) {
    (0, getSignParams_1.initLocalStorage)();
    const header = X_Data_1.window._webmsxyw(url, {});
    const params = await (0, getSignParams_1.getSignParams)(header);
    const xSCommon = getXSCommon(params);
    const result = {
        ...header,
        'X-s-common': xSCommon,
        cookie: X_Data_1.window.document.cookie.split(';')?.find(item => item.includes('a1'))?.split('=')[1],
    };
    return X_Data_1.window._webmsxyw(url);
}
exports.getXHeader = getXHeader;
