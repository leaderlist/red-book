...
function Sanji() {
    ...
    return function (_ace_4752e, _ace_aa949, _ace_be07c) {
        ...
        var _ace_75a05 = decode(_ace_aa949.b).split('').reduce(function (_ace_664b, _ace_b34db) {
            ...
            _ace_664b[_ace_664b.length - _ace_a72ba].push(-_ace_a72ba * 1 + _ace_b34db.charCodeAt(0));
            return _ace_664b;
        }, []);
        var _ace_aec23 = [function (p0, p1, p2, p3, p4, p5, p6) {
            ...
            function (p0, p1, p2, p3, p4, p5, p6) {
                var _ace_25a6 = _ace_ae44(p0, p1), _ace_d2389 = _ace_34d1(p0, p1);
            ...
            function (p0, p1, p2, p3, p4, p5, p6) {
                ...
                var _ace_bdcc = _ace_a3718.splice(_ace_a3718.length - _ace_163c1, _ace_163c1).map(_ace_8cba0), _ace_25a6 = _ace_a3718.pop(), _ace_8712 = _ace_8cba0(_ace_25a6);
            ...
            function (p0, p1, p2, p3, p4, p5, p6) {
                var _ace_25a6 = _ace_ae44(p0, p1), _ace_d2389 = _ace_34d1(p0, p1) - 1;
                _ace_25a6._ace_936[_ace_25a6._ace_47589] = _ace_d2389;
                _ace_1ae3c(_ace_d2389, _ace_be07c, _ace_be07c, 0);
                return ++p4;
            },
            ...
            function (p0, p1, p2, p3, p4, p5, p6) {
                var _ace_88c3 = _ace_34d1(p0, p1), _ace_d2389 = {};
            ...
            function (p0, p1, p2, p3, p4, p5, p6) {
                var _ace_25a6 = _ace_ae44(p0, p1), _ace_d2389 = _ace_34d1(p2, p3);
            ...
            , function (p0, p1, p2, p3, p4, p5, p6) {
                var _ace_404c = _ace_75a05.slice(_ace_34d1(p0, p1), _ace_34d1(p2, p3) + 1), _ace_71423 = _ace_66;
                _ace_1ae3c(function () {
                    _ace_420ea = {
...
function diffCookie(cookie) {
    const myCookieList = myCookie.replace(/\s*/g, "").split('; ').map(item => {
        return item.split('=');
    });
    const targetCookieList = cookie.replace(/\s*/g, "").split('; ').map(item => {
        return item.split('=');
    });
    console.log(myCookieList, targetCookieList);
    targetCookieList.forEach(tCookie => {
        const targetIndex = myCookieList.findIndex(item => item[0] === tCookie[0]);
        if (targetIndex === -1) {
            myCookieList.push(tCookie);
        }
        else if (myCookieList[targetIndex][1] !== tCookie[1]) {
            myCookieList[targetIndex] = tCookie;
        }
    });
    myCookie = myCookieList.map(item => item.join('=')).join(';');
}
...
const getXHeader = async (url, cookie, param) => {
    console.log(myCookie, 1111, document.cookie, 222, cookie);
    diffCookie(cookie);
    console.log(myCookie, 1111, document.cookie);
    initLocalStorage();
    const header = window._webmsxyw(url, param);
    const params = await getSignParams(header);
    const xSCommon = getXSCommon(params);
    const result = {
        ...header,
        'X-s-common': xSCommon,
